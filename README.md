...
* When you get a python3 error, it will also include `ModuleNotFoundError: No module named 'apt_pkg'`. Just ignore this bit, it has no effect on things running or not and stems from the fact that the installed version of python3 comes from a third-party PPA and is not [fully supported by Ubuntu](https://stackoverflow.com/questions/41720578/modulenotfounderror-in-tracebacks-with-python3-6-on-linux). Read the rest of the message to find out what's actually wrong.
