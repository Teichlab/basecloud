...
* **Docker**
* **Julia**
* **samtools, bcftools, biobambam2, bedtools, htop, parallel, sshfs**
...
* The Teichlab allocation on OpenStack was estimated for 10 active users with reasonable computational needs, assuming a `m1.2xlarge` instance per head with a 2TB volume to boot. As such, try not to use more than its resource total (26 cores, 236,600 MB RAM, 2TB volume space) at a time. If you do end up using more (dire computational straits happen), please free it up in a timely manner.
...
* Go onto [Zeta](http://zeta.internal.sanger.ac.uk) and log in with your Sanger user name and the password you just saved.
...
* Go to the Compute tab of [Zeta](http://zeta.internal.sanger.ac.uk), then Instances. Press Launch Instance. Go to the Key Pair tab. Press Import Key Pair. Paste in the bit you just copied into Public Key. Name the key pair something informative, such as your Sanger user ID. Press Import Key Pair.
...
* Log in to [Zeta](http://zeta.internal.sanger.ac.uk), go to the Compute tab, then Instances, press Launch Instance.
...
* Switch to Flavor. The flavours are a dropdown of available resource allocations you can request. Pick anything `m`, even the one-core `m1.tiny` works (and comes with 8GB RAM). If you need more cores to run something, by all means go for it! However, be mindful of your tools' abilities to actually effectively use cores and RAM, and if you go for heavier flavours be extra vigilant with removing idle instances.
...
* If you set up the Rstudio/jupyter notebook SSH tunnel, briefly disconnect from the Sanger network to kill it. If you're unwilling to do that, check whether the tunnel's active by calling `ps -a | grep ssh` on your computer. If you see an entry with a command that looks like the tunnel setup from this document, or whatever modifications you may have made to it, take note of the process ID in the first column of the results, and kill it with `kill -9 <id>`.
